...
def solveFile(file, dbclient):
	...
	for line in open(file):
		#logging.info(line)
		data = line.split("|")
		if len(data) <= 1 or data[3] == '未公开' or data[3].strip() == '':
...
def createInsertSql(data, bizPeriodStart, bizPeriodEnd, regCapital, establishDate):
	...
	values = " values ('" + data[3] + "','" + data[4] + "','" + filterVal(data[5]) + "','" + str(filterVal(regCapital)) + "','" + filterVal(data[9]) + "','" + filterVal(data[10]) + "','" + filterVal(data[11]) + "','" + filterVal(data[12]) + "','" + filterVal(data[13]) + "','" + filterVal(data[14]) + "','" + data[15].replace("\n", "") + "','" + data[0] + "','system'"
	if bizPeriodStart != '':
...
def createUpdateSql(data, bizPeriodStart, bizPeriodEnd, regCapital, establishDate):
	update = "update inc_enterprise t set t.legal_person = '" + filterVal(data[5]) + "', t.reg_capital = '" + str(filterVal(regCapital)) + "', t.reg_authority = '" + filterVal(data[9]) + "', t.address = '" + filterVal(data[10]) + "', t.phone = '" + filterVal(data[11]) + "', t.biz_reg_num = '" + filterVal(data[12]) + "', t.org_code = '" + filterVal(data[13]) + "', t.taxpayer_code = '" + filterVal(data[14]) + "', t.industry = '" + data[15] + "', t.area = '" + data[0]
	if bizPeriodStart != '':
